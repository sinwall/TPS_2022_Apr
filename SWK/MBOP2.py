    ...
    def reducer (self, X,save_trans_X=False): 
        ...
        for i in range(self.n_channels):
            ...
            if self.print_progress:
                ...
                print(f"Amount of reduction for {i} is: {self.red_rate[i,0]} to {self.red_rate[i,1]}")
            if save_trans_X:
    ...
    def transform(self, X,y=None,train_transform=False):
        ...
        if self.sparse:
            for i in range(self.n_channels):
                ...
                temp_col_list.append(temp_col)  #We reduce number of feautures by choosing non trivial patterns with indexing.
                del temp_col
            transform_X = hstack(temp_col_list).toarray()
        else:
            for i in range(self.n_channels):
                temp_col=self.MACHINES[i].transform(data_3d[:,:,i])
